...
import { View, Text, StyleSheet, ScrollView, TextInput, TouchableOpacity, ActivityIndicator, Alert, Image, Modal, KeyboardAvoidingView, Platform, RefreshControl } from 'react-native';
...
export default function ManagerDashboard() {
  ...
  useEffect(() => {
    if (!['manager', 'admin'].includes(user?.role || '')) {
      Alert.alert('Access denied', 'Manager access required', [{ text: 'OK', onPress: () => router.back() }]);
      return;
    }
...
function LeadsTab() {
  const [filter, setFilter] = useState<'all' | typeof LEAD_STATUSES[number]>('all');
  ...
  const load = useCallback(async () => {
    try {
      ...
      const r = await api.get(path); setItems(r.data);
  ...
  return (
    <View style={{ flex: 1 }}>
      ...
      {loading ? <ActivityIndicator color={theme.colors.primary} style={{ marginTop: 30 }} /> : (
        ...
        >
          {items.length === 0 ? <Text style={styles.empty}>No leads.</Text> : items.map(l => (
...
function ServiceTab() {
  const [filter, setFilter] = useState<'all' | typeof SR_STATUSES[number]>('all');
  const [items, setItems] = useState<any[]>([]);
  const [loading, setLoading] = useState(true);
  const [refreshing, setRefreshing] = useState(false);
  ...
  const load = useCallback(async () => {
    try {
      const path = filter === 'all' ? '/manager/service-requests' : `/manager/service-requests?status=${filter}`;
      const r = await api.get(path); setItems(r.data);
    } finally { setLoading(false); setRefreshing(false); }
  }, [filter]);
  useEffect(() => { load(); }, [load]);
  ...
  return (
    <View style={{ flex: 1 }}>
      <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.filterRow}>
        {(['all', ...SR_STATUSES] as const).map(f => (
          <TouchableOpacity key={f} onPress={() => setFilter(f as any)} style={[styles.chip, filter === f && styles.chipActive]}>
...
const styles = StyleSheet.create({
  ...
  refTxt: { fontSize: 11, color: theme.colors.textMuted, marginTop: 8 },
